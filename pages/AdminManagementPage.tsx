...
import { COUNTRIES, getCountryName, isValidCountryCode } from '../utils/countries';
...
const AdminManagementPage: React.FC<AdminManagementPageProps> = ({ entityType }) => {
    ...
    const [newCountry, setNewCountry] = useState('IT'); // Default to Italy
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        ...
        if (entityType === 'client') {
            ...
            if (!isValidCountryCode(newCountry)) {
                addToast('Please select a valid country', 'warning');
                return;
            }
        }
        ...
        try {
            if (entityType === 'client') {
                await api.addClient(newItemName, parseInt(newBusinessUnitId, 10), newCountry);
            } else {
    ...
    const handleUpdateItem = async () => {
        ...
        try {
            if (entityType === 'client') {
                ...
                if (!isValidCountryCode(client.paese)) {
                    addToast('Please select a valid country', 'warning');
                    return;
                }
                await api.updateClient(client.id, client.name, client.businessUnitId, client.paese);
            } else {
                        ...
                        <table className="w-full text-sm text-left text-slate-500 dark:text-slate-400">
                            <thead className="text-xs text-slate-700 dark:text-slate-300 uppercase bg-slate-100 dark:bg-slate-700/50">
                                <tr>
                                    ...
                                    {isClientManagement && <th scope="col" className="px-6 py-3 font-semibold">Country</th>}
                            ...
                            <tbody>
                                    ...
                                    <tr key={item.id} className={`border-b border-slate-200 dark:border-slate-700 ${index % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'} hover:bg-indigo-50 dark:hover:bg-slate-700/50`}>
                                        ...
                                        {isClientManagement && (
                                            <td className="px-6 py-4">
                                                {editingItem?.id === item.id ? (
                                                    <select
                                                        value={(editingItem as Client).paese || ''}
                                                        onChange={(e) => setEditingItem({ ...editingItem, paese: e.target.value })}
                                                        className="p-1 border rounded-md w-full bg-white dark:bg-slate-700 dark:border-slate-600"
                                                    >
                                                        <option value="" disabled>Select Country</option>
                                                        {COUNTRIES.map(country => (
                                                    ...
                                                    getCountryName((item as Client).paese) || <span className="text-slate-400">N/A</span>
                                                )}
                                            </td>
                                        )}
                                ...
                                {sortedAndFilteredItems.length === 0 && !loading && (
                                    <tr>
                                        <td colSpan={isClientManagement ? 5 : (isBuManagement ? 4 : 3)} className="text-center py-8 text-slate-500 dark:text-slate-400">
