...
const ForecastPage: React.FC = () => {
    ...
    const grandTotals = useMemo(() => {
        return groupedData.reduce((totals, buGroup) => ({
            totalDeclaredBudget: totals.totalDeclaredBudget + buGroup.totalDeclaredBudget,
            totalBudget: totals.totalBudget + buGroup.totalBudget,
            totalForecast: totals.totalForecast + buGroup.totalForecast,
        }), {
                        ...
                        <table className="w-full text-sm text-left text-slate-600 dark:text-slate-300 table-fixed">
                            ...
                            <tbody>
                                ...
                                {groupedData.length === 0 && !loading && (
                                    <tr><td colSpan={7} className="p-4"><div className="flex flex-col items-center p-12 bg-slate-50 dark:bg-slate-800/50 rounded-lg text-center"><div className="p-4 bg-indigo-100 dark:bg-slate-700 rounded-full"><TableCellsIcon className="w-12 h-12 text-indigo-400"/></div><h3 className="mt-4 text-lg font-medium text-slate-800 dark:text-slate-100">No forecast data</h3><p className="mt-2 text-slate-500 dark:text-slate-400">No forecast entries match your current filters.</p></div></td></tr>
                                )}
                                {/* Total Row */}
                                {groupedData.length > 0 && (
                                    <tr className="bg-indigo-50 dark:bg-indigo-900/30 border-t-2 border-indigo-200 dark:border-indigo-700 font-bold">
                                        <td className="px-6 py-4 font-bold text-indigo-800 dark:text-indigo-200" colSpan={2}>
                                            <div className="flex items-center">
                                                <span className="text-lg">TOTALE</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 text-right font-bold text-indigo-800 dark:text-indigo-200 text-lg">
                                            {formatCurrency(grandTotals.totalDeclaredBudget)}
                                        </td>
                                        <td className="px-6 py-4 text-right font-bold text-indigo-800 dark:text-indigo-200 text-lg">
                                            {formatCurrency(grandTotals.totalBudget)}
                                        </td>
                                        <td className="px-6 py-4 text-right font-bold text-indigo-800 dark:text-indigo-200 text-lg">
                                            {formatCurrency(grandTotals.totalForecast)}
                                        </td>
                            ...
                            {clientsLoading ? (
                                ...
                                <>
                                    ...
                                    {availableClients.map(client => {
                                        const businessUnit = businessUnitsMap.get(client.businessUnitId || 0);
                                        const businessUnitName = businessUnit?.name || 'N/A';
                                        return (
                                            <option key={client.id} value={client.id}>
                                                {client.name} - {businessUnitName}
                                            </option>
                                        );
                                    })}
                                </>
