...
const DashboardPage: React.FC = () => {
    ...
    }, [forecasts, dateRange, selectedBuIds]);

    const summaryData = useMemo(() => {
        const budget = forecastsInRange.reduce((sum, f) => sum + f.budget, 0);
        const forecast = forecastsInRange.reduce((sum, f) => sum + f.forecast, 0);
    ...
    const barChartData = useMemo(() => {
        const dataMap = new Map<string, { budget: number, forecast: number }>();
        forecastsInRange.forEach(f => {
            const buName = businessUnits.find(bu => bu.id === f.businessUnitId)?.name || 'Unknown';
            const current = dataMap.get(buName) || { budget: 0, forecast: 0 };
            current.budget += f.budget;
            current.forecast += f.forecast;
    ...
    const lineChartData = useMemo(() => {
        const monthDataMap = new Map<string, { budget: number, forecast: number }>();
    
        // Populate map with data from forecasts in range
        forecastsInRange.forEach(f => {
            const monthKey = `${f.year}-${String(f.month).padStart(2, '0')}`;
            const current = monthDataMap.get(monthKey) || { budget: 0, forecast: 0 };
            current.budget += f.budget;
            current.forecast += f.forecast;
        ...
        while (iterDate <= dateRange.end) {
            ...
            const data = monthDataMap.get(monthKey) || { budget: 0, forecast: 0 };
            result.push({
